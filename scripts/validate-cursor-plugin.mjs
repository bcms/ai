...
/**
 * Validates ai/.cursor-plugin/marketplace.json and bundled Cursor plugins.
 * Run from any directory: node scripts/validate-cursor-plugin.mjs
 */
...
async function main() {
  ...
  for (const [index, entry] of marketplace.plugins.entries()) {
    ...
    const hooksConfigured = extractPathValues(pluginManifest.hooks).length > 0;
    if (hooksConfigured && !(await pathExists(hooksPath))) {
      addWarning(`${entry.name}: hooks configured in plugin.json but hooks/hooks.json is missing.`);
    }

    const mcpPath = path.join(pluginDir, "mcp.json");
    const mcpExamplePath = path.join(pluginDir, "mcp.json.example");
    if (!(await pathExists(mcpPath)) && !(await pathExists(mcpExamplePath))) {
